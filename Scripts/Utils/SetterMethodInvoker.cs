...
{
    [Serializable]
    public class SetterMethodInvoker
    {
        [Serializable]
        ...
        public bool useLocal;
        public GameObject variableTarget;
        public Value returnVariable;
        ...
        public IEnumerator Invoke(MonoBehaviour mono)
        {
            ...
            if (m != null)
            {
                ...
                if (nonObject)
                {
                    rv = m.Invoke(null, objs.ToArray());
                    CreateVariable(rv);
                    yield return null;
                }
                else
                {
                    rv = m.Invoke(invokeObject, objs.ToArray());
                    CreateVariable(rv);
                    yield return null;
                }
            }
        ...
        public List<string> GetInvokeObjectMethodNames()
        {
            ...
            {
                if (SupportReturnType(m.ReturnType))
                {
                    var nsp = false;
        ...
        private bool SupportReturnType(Type type)
        {
            switch (type.Name)
            {
                case "Single":
        ...
        private void CreateVariable(object value)
        {
            ...
            {
                case "Single":
                    ((FloatValue) returnVariable).value = (float) value;
                    break;

                case "Boolean":
                    ((BoolValue) returnVariable).value = (bool) value;
                    break;

                case "String":
                    ((StringValue) returnVariable).value = (string) value;
                    break;

                case "Int32":
                    ((IntValue) returnVariable).value = (int) value;
                    break;

                case "Vector2":
                    ((Vector2Value) returnVariable).value = (Vector2) value;
                    break;

                case "Vector3":
                    ((Vector3Value) returnVariable).value = (Vector3) value;
                    break;

                case "Vector4":
                    ((Vector4Value) returnVariable).value = (Vector4) value;
                    break;

                case "Object":
                    ((ObjectValue) returnVariable).value = (Object) value;
                    break;

                case "GameObject":
                    ((GameObjectValue) returnVariable).value = (GameObject) value;
                    break;
            }
        }
    }
